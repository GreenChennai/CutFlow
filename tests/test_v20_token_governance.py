...
def _mk_proj(tmp_path: Path) -> Path:
    root = tmp_path / "proj"
    for d in ("00_brief", "01_materials", "03_assets", "05_ir"):
        (root / d).mkdir(parents=True)
    (root / "00_brief" / "cards.json").write_text(
    ...
    (root / "05_ir" / "project.json").write_text(json.dumps({
        ...
        "tracks": [{"kind": "video", "name": "main", "clips": [
            {"src": "01_materials/a.mp4", "startMs": 0, "durationMs": 12000,
             "sourceInMs": 0}]}],
...
def test_t1_gen_cards_deterministic_idempotent_and_registered(tmp_path):
    ...
    r1 = _run("rs_artboard.py", "gen-cards", "--from", "00_brief/cards.json", cwd=root)
    assert r1.returncode == 0, r1.stderr
    doc1 = _last_json(r1)
    assert doc1["ok"] and doc1["code"] == "CARDS_GENERATED"
    html1 = (root / "03_assets/artboard/c01/src/index.html").read_text(encoding="utf-8")
    assert "什么是 GEO" in html1 and "1080" in html1
    man = json.loads((root / "03_assets/artboard/manifest.json").read_text(encoding="utf-8"))
    it = {i["id"]: i for i in man["items"]}["c01"]
    # P6 口径:artboard 目录在工程根下 → 清单字段写工程根相对,--export/--apply 默认 --root 即命中
    assert it["output"] == "03_assets/artboard/c01/export/c01.png"
    assert it["project"] == "03_assets/artboard/c01/src"
    assert it["sourceHash"]

    # 再跑一次:字节级一致(确定性),manifest 也不变(幂等)
    r2 = _run("rs_artboard.py", "gen-cards", "--from", "00_brief/cards.json", cwd=root)
    assert r2.returncode == 0 and _last_json(r2)["data"]["kept"] == ["c01", "c02"]
    html2 = (root / "03_assets/artboard/c01/src/index.html").read_text(encoding="utf-8")
    man2 = (root / "03_assets/artboard/manifest.json").read_text(encoding="utf-8")
    assert html1 == html2 and man2 == (root / "03_assets/artboard/manifest.json").read_text(encoding="utf-8")


def test_t1_gen_cards_refuses_clobbering_without_force(tmp_path):
    """手改卡片不静默冲掉:与计划不一致即 CARD_EXISTS 非零退出,--force 才覆盖。"""
    root = _mk_proj(tmp_path)
    assert _run("rs_artboard.py", "gen-cards", "--from", "00_brief/cards.json", cwd=root).returncode == 0
    target = root / "03_assets/artboard/c01/src/index.html"
    target.write_text("<!-- 手改过的卡片 -->", encoding="utf-8")
    r = _run("rs_artboard.py", "gen-cards", "--from", "00_brief/cards.json", cwd=root)
    assert r.returncode == 2 and _last_json(r)["code"] == "CARD_EXISTS"
    assert target.read_text(encoding="utf-8").startswith("<!-- 手改")
    r2 = _run("rs_artboard.py", "gen-cards", "--from", "00_brief/cards.json",
              "--force", cwd=root)
...
def test_t1_gen_cards_bad_plans_are_hard_fail(tmp_path):
    root = _mk_proj(tmp_path)
    (root / "00_brief" / "bad1.json").write_text("[]", encoding="utf-8")
    (root / "00_brief" / "bad2.json").write_text(
        json.dumps([{"id": "x", "title": "t", "role": "sfx"}], ensure_ascii=False), encoding="utf-8")
    r1 = _run("rs_artboard.py", "gen-cards", "--from", "00_brief/bad1.json", cwd=root)
    assert r1.returncode == 2 and _last_json(r1)["code"] == "BAD_PLAN"
    # P8 同源纪律:白名单外字段直接报错,不静默吞(role 曾是幽灵字段的教训)
    r2 = _run("rs_artboard.py", "gen-cards", "--from", "00_brief/bad2.json", cwd=root)
...
def test_t1_export_must_not_skip_when_product_missing(tmp_path, monkeypatch):
    ...
    man_path = root / "03_assets/artboard/manifest.json"
    calls: list[str] = []
    ...
    (root / "03_assets/artboard/c01/export/c01.png").unlink()
    monkeypatch.setattr(sys, "argv", ["rs_artboard.py", str(man_path), "--export",
                                      "--root", str(root)])
    code, _, doc = _capture(ab.main)
...
def test_t1_export_fallback_engine_plumbing(tmp_path, monkeypatch):
    ...
    assert _run("rs_artboard.py", "gen-cards", "--from", "00_brief/cards.json", cwd=root).returncode == 0
    man = root / "03_assets/artboard/manifest.json"
    monkeypatch.setattr(ab, "_load_artboard_config", lambda: {"artboard_dir": str(art)})
    ...
    out = root / "03_assets/artboard/c01/export/c01.png"
...
def test_t1_add_overlay_mounts_track_usedin_and_manual_edit(tmp_path):
    ...
    assert _run("rs_artboard.py", "gen-cards", "--from", "00_brief/cards.json", cwd=root).returncode == 0
    for cid in ("c01", "c02"):
        out = root / "03_assets/artboard" / cid / "export" / f"{cid}.png"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_bytes(PNG_1PX)
    r = _run("rs_ir.py", "add-overlay", "05_ir/project.json",
             "--manifest", "03_assets/artboard/manifest.json",
             "--plan", "00_brief/cards.json", cwd=root)
    ...
    assert clips[0]["src"] == "03_assets/artboard/c01/export/c01.png"
    ...
    r2 = _run("rs_ir.py", "add-overlay", "05_ir/project.json",
              "--manifest", "03_assets/artboard/manifest.json",
              "--plan", "00_brief/cards.json", cwd=root)
    assert r2.returncode == 2 and _last_json(r2)["code"] == "OVERLAY_ISSUES"
    (root / "00_brief" / "swap.json").write_text(json.dumps([
        {"card": "c02", "startMs": 100, "durationMs": 900}], ensure_ascii=False), encoding="utf-8")
    r3 = _run("rs_ir.py", "add-overlay", "05_ir/project.json",
              "--manifest", "03_assets/artboard/manifest.json",
              "--plan", "00_brief/swap.json", "--replace", cwd=root)
...
def test_t1_add_overlay_hard_fails_on_missing_product_and_overlap(tmp_path):
    ...
    assert "manualEdit" not in (root / "05_ir/project.json").read_text(encoding="utf-8")
    # 产物补齐后:时间窗重叠 → 硬失败(重叠检测在产物校验之后仍会拦)
    for cid in ("c01", "c02"):
        out = root / "03_assets/artboard" / cid / "export" / f"{cid}.png"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_bytes(PNG_1PX)
    (root / "00_brief" / "overlap.json").write_text(json.dumps([
        {"card": "c01", "startMs": 1000, "durationMs": 5000},
        {"card": "c02", "startMs": 2000, "durationMs": 1000}], ensure_ascii=False), encoding="utf-8")
    r2 = _run("rs_ir.py", "add-overlay", "05_ir/project.json",
              "--manifest", "03_assets/artboard/manifest.json",
              "--plan", "00_brief/overlap.json", cwd=root)
    assert r2.returncode == 2 and _last_json(r2)["code"] == "OVERLAY_ISSUES"
...
def test_t2_capabilities_is_short_and_machine_readable():
    ...
    assert len(raw.splitlines()) < 1200, "能力目录过长(细节应留给 --help)"
...
def test_t1_full_chain_without_ad_hoc_scripts(tmp_path):
    ...
    r1 = _run("rs_artboard.py", "gen-cards", "--from", "00_brief/cards.json", cwd=root)
    assert r1.returncode == 0, r1.stderr
    r2 = _run("rs_artboard.py", "export-fallback", "03_assets/artboard/manifest.json",
              cwd=root)
    assert r2.returncode == 0, r2.stdout + r2.stderr
    png = root / "03_assets/artboard/c01/export/c01.png"
