...
def test_end_to_end_3x4_pipeline(tmp_path):
    ...
    assert (root / "01_原始素材" / "manifest.json").is_file()

    # 4) S1 对齐(--src 把 wordline.source 指到真实素材,否则 IR 会拿转写稿当视频)
    _run("rs_align.py", "build", "--from-transcript", "02_转写与校对/transcript.json",
         "--src", "01_原始素材/sample.mp4", "--out", "05_时间线工程/wordline.json", cwd=root)

    # 5) S2 粗剪(带 --media 走音频能量探测)
    cut = _run("rs_cut.py", "05_时间线工程/wordline.json", "--detect", "all", "--out", "04_粗剪决策",
               "--media", "01_原始素材/sample.mp4", cwd=root)
    assert "srcTotalMs" in cut["data"]
    _run("rs_cut.py", "--apply", "04_粗剪决策/cutlist.json", cwd=root)

    # 6) 重映射 + S3 IR
    _run("rs_align.py", "remap", "05_时间线工程/wordline.json",
         "--cutlist", "04_粗剪决策/cutlist.applied.json", "--out", "05_时间线工程/wordline.final.json", cwd=root)
    _run("rs_ir.py", "build", "--from-cutlist", "04_粗剪决策/cutlist.applied.json",
         "--slug", "e2e", "--ratio", "3x4", "--out", "05_时间线工程/project.json", cwd=root)
    ...
    sub = _run("rs_subtitle.py", "--from-wordline", "05_时间线工程/wordline.final.json",
               "--platform", "xiaohongshu", "--out", "06_成片输出", cwd=root)
    ...
    # 9) S8 渲染(3:4 画幅真出片)
    mp4 = list((root / "06_成片输出").glob("*.mp4"))
    _run("rs_render.py", "05_时间线工程/project.json", "--ratio", "3x4", "--profile", "draft", cwd=root)
    mp4 = sorted(p for p in (root / "06_成片输出").glob("*.mp4"))
    assert mp4, "必须产出成片"
