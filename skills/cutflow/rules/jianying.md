...
`rs_jy_draft.py 05_时间线工程/project.json --name <草稿名> [--subtitles 03_创作素材/tts/manifest.json] [--open]` / 预检:`rs_jy_draft.py 05_时间线工程/project.json --dry-run`
...
- 落位 `<工程根>/05_时间线工程/导出/剪映59/<名>/draft_content.json` + `draft_meta_info.json`(半成品出口落点,ADR-0052),并注册剪映 root_meta_info.json(首页可见,条目指回工程区落点);写后自动回读对账(`DRAFT_GATE_FAIL` = 落盘与计划不符)。
...
- 交付时告诉用户:在剪映首页找到同名草稿即可继续精修;**精修成果请在剪映里直接导出**——草稿改动不会回流工程区(单向出口,`rules/editing-roundtrip.md`)。
...
确认分辨率/帧率 → 点导出 → 等待完成(默认 20min 超时)→ 产物在导出目录,移入 06_成片输出/。
