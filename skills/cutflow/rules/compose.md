...
> v0.14(ADR-0031):**抠像与背景合成已从 CutFlow 移除**。主轨素材须是用户预先抠好并合成背景的成片;本册只讲剪辑。IR 里出现 `chroma`/`background` 字段会被 `rs_ir validate` 直接报错。

## IR 书写(templates/project.schema.json)

- 毫秒单位;canvas 1080x1920 或 1920x1080;fps 30 默认。
- tracks[0] 必须是主视频轨(顺序拼接);后续 video 轨是画中画/信息卡(支持 png/jpg 静图 / 视频叠加)。
...
> v0.14(ADR-0031)已移除抠像/背景合成,本表 v0.6–v0.13 的绿幕键控战报(色度采样/幽灵人物/白墙裁切)不再适用于新管线,历史实现见 git 与 ADR-0029(已作废)。幕布素材改由 S0 检测阻断(见 `rules/intake.md`)。
