...
| **DP 字幕断句** | "滚滚长/江东逝水"式断错 | 约束最优 DP + 禁切表(词内/虚词后/数量词/标点前),竖屏 10–12 字/卡,CPS ≤9,Netflix 时长规范 |
| **卡拉OK 逐字字幕** | 口播类没有逐字跟唱感 | `--karaoke` 按字级时间戳生成 ASS `\kf` 染色,禁插值、只认 pkg 后端真实字级 |
| **seg 级渲染缓存 + retext 回灌** | 改个字/校对一句要全片重渲 | 段级内容寻址缓存 + 步骤门禁,JJAV2815 实测改字幕重出片 174s→54s;校对稿 `rs_align retext` 直接回灌 wordline |
...
设计决策见 `docs/adr/`(0001–0017),迭代分析见 `docs/OPTIMIZATION-v4.md`(对齐/粗剪/增量/一条龙)、`docs/OPTIMIZATION-v5.md`(自带 ASR/分级验证/一键重建/artboard 闭环)与 `docs/OPTIMIZATION-v6.md`(seg 缓存/retext 回灌/卡拉OK);真实素材实测战报见 `skills/cutflow/rules/compose.md`。
