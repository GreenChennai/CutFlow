...
> **AI 视频总控技能**:口播/文案/分镜 → 转写对齐 → 粗剪 → 合成 → 字幕 → 烧录 → 分级自检 → 封面文案,一条龙直出成片与平台物料。
> **同伴项目 [CutForge](https://github.com/GreenChennai/cutforge)**(Rust 编辑器内核):CutFlow 管 S0–S11 批量机械臂,CutForge 管"人的手"——时间线交互、标注、AI 可 diff 的编辑;两者读写同一份工程文件,经 `rs_editor/rs_notes/rs_oplog/rs_gate` 四桥互通。

## 素材进,成片出。 让 AI Agent 端到端制作视频:接收口播视频 / 文案 / 剧本分镜 / 图片,经
「转写对齐 → 粗剪 → 合成 → 剪辑 → 字幕 → 烧录 → 体检自检 → 封面文案」产出成片与平台物料,
并同步生成**可在剪映 5.9 继续精修的草稿工程**。

- **视频类型**:`talking-head`(纯口播,素材须已抠像合成背景)/ `talking-head+animation`(口播+动画卡)/ `pure-animation`(纯动画),每类有专属管线手册;画幅 9:16 / 3:4 / 16:9,**平台字幕预设**(抖音 / 视频号 / 小红书 / B站),多 Logo × 多比例**变体矩阵**
...
| **绿幕检测门禁**(v0.14) | 未预处理的绿幕素材被剪进成片 | S0 抽帧检测幕布并在交付前阻断(ADR-0031):CutFlow 不再抠像——用户须先自行抠像+合成背景;检测命中即提示处理,误判可由用户说明后 `green-ok` 留痕放行 |
| **能量校准**(v0.13) | 字幕整体抢拍/拖拍 | S1 自动按音频能量包络校准字级时间轴(ADR-0030):系统性偏移实测 140ms→0ms;纯 stdlib 实现,静音/损坏素材显式降级 |
...
| **成片体检 QC**(v0.11) | 黑帧/冻结/音画漂移/响度超标要人眼看 | `rs_sync --qc` 机械闸:黑帧≥0.3s / 冻结≥2.5s / VFR 混帧 / 响度(I -14 LUFS ±1、TP -1 dBTP,双 pass loudnorm)直接 FAIL |
...
- **S1 之后自动执行能量校准**(v0.13):`rs_align calibrate` 按音频能量起点校准字级时间轴,系统性偏移在门禁前被修掉;`wordline.calibration` 留痕可审计。
- **S9 是三重机械闸**:①字幕↔Wordline 对齐断言(中位 ≤40ms);②成片音频内容闸(ASR 对账:片头句唯一/相似度 ≥0.90/无重复段);③QC 体检(黑帧/冻结/VFR/响度)。**S0 幕布检测**(ADR-0031)另在开工前阻断未预处理的绿幕素材。
...
```
S0 素材摄取(幕布检测门禁)→ S1 自带ASR转写+字级对齐(Wordline 唯一真相源) → S2 粗剪(七类检测器+guard+删改稿)
     → S3-S6 合成(IR:转场三级语法/punch-in/TTS音色卡/artboard卡/Logo变体/音效)
     → 剪辑双出:FFmpeg 七步管线(段缓存/双pass响度/字幕最后叠)
                  + 剪映 5.9 明文草稿(可编辑工程)
