...
## 🆕 v0.20 · v2 迭代:剪得好看——素材库 / 转场特效 / 效果语法

> 依据《CutFlow-迭代计划-v2-20260925》(M11–M15,ADR-0053~0059)。测试 783 → 948+ 全绿。

- **素材库 119 条**(sfx 40 / 元素贴图 49 / 花字 14 / BGM 16),每条**许可四字段**(来源/许可/可否商用/归因)机器可读,交付自动生成 `素材归因.md`;字体链接 artboard 不再硬编码;
- **效果目录 317 条**:可执行档 **186**(T1 FFmpeg 原生 162 + T2 GLSL 24),剪映名规范化全量登记,「用不上的不用做」逐条带 why;
- **T2 GLSL 转场真渲**(B3:moderngl + gl-transitions,只渲转场重叠区零漂移),缺失/关闭自动降级最接近 T1 并留痕;
- **效果真的会被用上**:`rs_intent` 产 effectsPlan → `rs_edit context` 显示「本工程该用的特效」进度与缺口 → **九判据门禁**(EFFECTS_UNUSED 拦「有特效却一处不用」、EFFECTS_OVERUSED 拦滥用);每个风格包带效果处方(纯口播 flashy_max=0,混剪 flashy≤3);
- **零静默吞能力**(ADR-0055):schema 声明的每个枚举值渲染端要么实现、要么删除——motion 五个空壳全部真实现(缩放入场/滑入滑出),bgm.loop 生效,全枚举机械审计门禁;
- **缺陷止血**(分册03):markers 三方分歧/转场 cut·none 校验矛盾/Logo 安全区双口径三个高危全修,子进程超时补齐,中文路径端到端入测;
- **渲染提速**:段级/变体级并行(`--jobs`,产物与串行字节级一致)+ ffprobe 探测合并(≈3N→N)。
